...
@interface SZPhotoGraphVC (RecordManagerDelegate)<RecordManagerDelegate>

@end
@implementation SZPhotoGraphVC
...
- (void)viewDidLoad
{
    ...
    //设置预览层
    CGRect previewFrame=CGRectMake(0, 0, SCREENWIDTH, SCREENHEIGTH-2*44);
    UIView* previewView=[[UIView alloc]initWithFrame:previewFrame];
    ...
    CGFloat margin=(SCREENWIDTH-btnWidth*3)/4;
    //1.拍摄
    UIButton* recordButton=[[UIButton alloc] initWithFrame:CGRectMake(margin, previewFrame.size.height, btnWidth, btnHeigth)];
    
    [recordButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [recordButton setTitle:NSLocalizedString(@"Shoot", nil) forState:UIControlStateNormal];
    [recordButton addTarget:self action:@selector(toggleRecording) forControlEvents:UIControlEventTouchUpInside];
    self.recordButton=recordButton;
    [self.view addSubview:self.recordButton];
    //2. 切换镜头
    UIButton* cameraToggleButton=[[UIButton alloc] initWithFrame:CGRectMake(2*margin+btnWidth, previewFrame.size.height, btnWidth, btnHeigth)];
    [cameraToggleButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [cameraToggleButton addTarget:self action:@selector(toggleCamera) forControlEvents:UIControlEventTouchUpInside];
    [cameraToggleButton setTitle:NSLocalizedString(@"Switch", nil) forState:UIControlStateNormal];
    self.cameraToggleButton=cameraToggleButton;
    [self.view addSubview:cameraToggleButton];
    //3.拍照
    UIButton* stillButton=[[UIButton alloc] initWithFrame:CGRectMake(3*margin+2*btnWidth, previewFrame.size.height, btnWidth, btnHeigth)];
    [stillButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [stillButton addTarget:self action:@selector(captureStillImage) forControlEvents:UIControlEventTouchUpInside];
    [stillButton setTitle:NSLocalizedString(@"PhotoGraph", nil) forState:UIControlStateNormal];
...
///
@implementation SZPhotoGraphVC (RecordManagerDelegate)
//拍照或者录制视频出错时调用的方法
